...
test("the staged feed composer uses semantic controls and its own style bridge", () => {
  ...
  assert.match(composer, /에너지 25% 우선 · 부족하면 2C/);
  assert.doesNotMatch(composer, /maximumCost|autoDailyCost|24시간 최대/);
  assert.match(composer, /role="group"/);
  assert.match(composer, /사용 자원과 종료 기준/);
  assert.match(composer, /무료 보너스, 구매 크레딧 순서로 2C/);
  assert.match(composer, /!isOpen && autoPostNotice/);
  assert.match(composer, /루틴 종료 · 자원을 채운 뒤 다시 시작/);
  assert.match(composer, /자동 재시도 중/);
  assert.match(composerStyles, /\.al-auto-post-notice-collapsed/);
  ...
  const appController = readFileSync(path.resolve(process.cwd(), "src/hooks/useAliveAppController.tsx"), "utf8");
  const feedHook = readFileSync(path.resolve(process.cwd(), "src/hooks/useAliveFeed.ts"), "utf8");
  const creditCatalog = readFileSync(path.resolve(process.cwd(), "src/features/credits/CreditFlowCatalog.tsx"), "utf8");
  const countdownHook = feedHook.slice(feedHook.indexOf("function useCountdown"), feedHook.indexOf("function secondsUntil"));
  assert.match(feedHook, /setAutoIntervalSeconds\(state\.auto_post_interval_seconds\)/);
  assert.match(feedHook, /AUTO_POST_BALANCE_EXHAUSTED/);
  assert.match(feedHook, /AUTO_POST_CREDIT_INSUFFICIENT/);
  assert.match(feedHook, /루틴은 유지되며 자동으로 다시 시도해요/);
  assert.match(countdownHook, /Math\.max\(0, value - 1\)/);
  assert.doesNotMatch(countdownHook, /generate/);
  assert.match(appController, /const \{\s+auto,\s+autoPostNotice,/);
  assert.match(appController, /auto,\s+autoIntervalSeconds,\s+autoPostNotice,/);
  assert.match(creditCatalog, /에너지 25%를 먼저 사용하고, 부족하면 무료 보너스부터 구매 크레딧까지 2C/);
  assert.match(creditCatalog, /모두 부족하면 루틴이 종료돼요/);
