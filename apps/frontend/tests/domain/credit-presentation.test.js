...
test("credit flow metadata makes every DM response tier available", () => {
  ...
  assert.equal(creditFlowMeta("direct_dm_pro_story").available, true);
...
test("DM credit preview makes the next payment source and shortage explicit", () => {
  ...
  const pro = { code: "direct_dm_pro", label: "중요한 답장", energy_percent: 25, credits: 5, energy_eligible: false, bonus_eligible: false };
  assert.deepEqual(creditUsagePreview(pro, { energy_percent: 100, purchased_credits: 5, total_credits: 55 }), { action: "none", detail: "이번 답장에 5C 사용 예상", label: "구매 크레딧 5C", state: "credits" });
  assert.deepEqual(creditUsagePreview(pro, { energy_percent: 100, purchased_credits: 0, total_credits: 55 }), { action: "open-credits", detail: "중요한 답장은 구매 C만 사용해요 · 5C 필요, 현재 0C", label: "구매 크레딧이 부족해요", state: "insufficient" });
});
