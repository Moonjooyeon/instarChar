...
export const SYMMETRIC_RELATION_BASE = [
...
export function isLoveRelation(label) {
  return LOVE_RELATIONS.test(label || "");
}

export function dirKey(from, to) {
  return `${from}>${to}`;
}
...
function singleTokenMatches(a, b) {
...
export function relationBaseFromLabel(label) {
  return baseFromLabel(label, RELATION_BASE);
}

export function symmetricRelationBaseFromLabel(label) {
  return baseFromLabel(label, SYMMETRIC_RELATION_BASE);
}

function baseFromLabel(label, rules) {
  if (!label) return null;
  const hit = rules.find(([matcher]) => matcher.test(label));
  return hit ? hit[1] : null;
}

export function affinityWithBase(stored, base) {
...
export function attachStage(value) {
...
export function relationStageLabel(label, affinity) {
...
export function relationLabelFromAffinity(value, current = "") {
...
function positiveRelationLabel(value) {
  if (value >= 35) return "호감";
  return value >= 15 ? "관심" : "아는 사이";
}

function negativeRelationLabel(value) {
...
export function intimacyBoundaryRules({ speakerName, listenerName, affinityValue = 0, relationHint = "", messageCount = 0 } = {}) {
