...
- 숫자 하나만 출력. -30 ~ +8 범위 정수. 설명·기호 금지. 예: 5 또는 -20`;
  try {
    const raw = (await postGenerateContent({ flow: "assist_session", model: MODEL_UTIL, max_tokens: 10, system: sys, messages: [{ role: "user", content: transcript }] }, "호감도 판정 API")).trim();
...
기억할 게 없으면 빈 배열.`;
  const raw = await postGenerateContent({ flow: "assist_session", model: MODEL_UTIL, max_tokens: 2048, system: sys, messages: [{ role: "user", content: transcript }] }, "기억 통합 API");
