...
export function useAliveSessionAnalysis({
  ...
  async function judgeSession(aName, bName, lines) {
    ...
    const judgeOne = async (from, to) => {
      const delta = await judgeAffinityDelta(from, to, transcript, readApiContent);
  ...
  async function processSession(aName, bName, lines, memOnly, roomKey = "") {
    ...
    try {
      const obj = await analyzeSessionSummary(aName, bName, transcriptFromLog(log, 16), readApiContent);
      applySessionAffinity({ bName, bumpAffinity, bumpRoomAffinity, flags, log, memOnly, obj, roomKey, aName });
      if (flags.aMem) saveMemories(aName, bName, cleanMemItems(obj.mem_a || []), roomKey);
      if (flags.bMem) saveMemories(bName, aName, cleanMemItems(obj.mem_b || []), roomKey);
    } catch (e) {
...
- 숫자 하나만 출력. -30 ~ +8 범위 정수. 설명·기호 금지. 예: 5 또는 -20`;
  try {
    const res = await fetch("/api/generate", {
      ...
      body: JSON.stringify({ model: MODEL_UTIL, max_tokens: 10, system: sys, messages: [{ role: "user", content: transcript }] }),
    });
...
function sessionFlags(aName, bName, isPersonaName) {
  const OWNER = "나";
  const aPersona = isPersonaName(aName);
  const bPersona = isPersonaName(bName);
...
async function analyzeSessionSummary(aName, bName, transcript, readApiContent) {
...
기억할 게 없으면 빈 배열.`;
  const res = await fetch("/api/generate", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model: MODEL_UTIL, max_tokens: 2048, system: sys, messages: [{ role: "user", content: transcript }] }),
  });
  const raw = await readApiContent(res, "기억 통합 API");
...
function applySessionAffinity({ aName, bName, bumpAffinity, bumpRoomAffinity, flags, log, memOnly, obj, roomKey }) {
  if (memOnly) return;
...
function applyAffinityDelta(bumpAffinity, from, to, value, log) {
  const delta = normalizedDelta(value);
  if (delta !== 0) bumpAffinity(from, to, delta, log.map((item) => `${item.who}: ${item.text}`));
}

function applyRoomAffinityDelta(bumpRoomAffinity, roomKey, from, to, value) {
  const delta = normalizedDelta(value);
  if (delta !== 0) bumpRoomAffinity(roomKey, from, to, delta);
}

function normalizedDelta(value) {
  return Math.max(-30, Math.min(8, parseInt(value, 10) || 0));
}
