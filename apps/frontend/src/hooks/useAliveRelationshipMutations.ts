...
export function useAliveRelationshipMutations({
  ...
  async function triggerProposal(askerName: string, otherName: string): Promise<void> {
    ...
    try {
      ...
      const text = (await postAssistContent({ kind: "relationship_proposal", idempotency_key: idempotencyKey, context: sys, messages: [{ role: "user", content: `(${askerName}가 오너에게 ${otherName}에 대한 마음을 털어놓으며 허락을 구한다.)` }] }, "관계 제안 API")).trim();
  ...
  async function judgeAcceptance(askerName: string, otherName: string): Promise<boolean> {
    ...
    try {
      ...
      const raw = (await postAssistContent({ kind: "relationship_judge", idempotency_key: idempotencyKey, context: acceptancePrompt({ askerName, back, otherChar, otherName }), messages: [{ role: "user", content: "판정:" }] }, "관계 판정 API")).toUpperCase();
