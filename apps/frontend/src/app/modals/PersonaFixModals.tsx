      ...
      {USER_PERSONA_FEATURE_ENABLED && canUseApp && personaDraft && (
        <div className="al-modal-bg" onClick={() => setPersonaDraft(null)}>
          <div className="al-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <textarea className="al-pd-input area" placeholder="성격·배경 (어떤 사람인지)" value={personaDraft.persona} onChange={(event) => setPersonaDraft({ ...personaDraft, persona: event.target.value })} />
            <input className="al-pd-input" placeholder="말투 (예: 나른한 반말, 존댓말…)" value={personaDraft.speech} onChange={(event) => setPersonaDraft({ ...personaDraft, speech: event.target.value })} />
...
function PersonaActions({ ctx }) {
  ...
  return (
    <div className="al-pd-btns">
      {personaDraft.id && (
        <button className="al-pd-del" onClick={() => { deletePersona(personaDraft.id); setPersonaDraft(null); }}>삭제</button>
      )}
    ...
    <div className="al-modal-bg" onClick={() => setFixTarget(null)}>
      <div className="al-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="al-fixchips">
          {QUICK_FIXES.map((quickFix) => <button key={quickFix} className="al-fixchip" onClick={() => setFixText((text) => text ? `${text}, ${quickFix}` : quickFix)}>{quickFix}</button>)}
        </div>
        <textarea className="al-fixinput" value={fixText} onChange={(event) => setFixText(event.target.value)} placeholder="예: 얘는 이럴 때 더 무심하게 말해. 느낌표 안 씀." />
        <div className="al-modal-actions">
          <button className="al-modal-cancel" onClick={() => setFixTarget(null)}>취소</button>
          <button className="al-modal-saveonly" disabled={!fixText.trim()} onClick={() => { addCorrection(fixText, fixTarget.who); setFixTarget(null); }}>교정만</button>
          <button className="al-modal-save" disabled={!fixText.trim()} onClick={() => {
