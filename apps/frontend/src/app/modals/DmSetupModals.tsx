        ...
        <div className="al-modal-bg" onClick={() => setDmSettingsOpen(false)}>
          <div className="al-world-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="al-world-options compact">
              <button className={dmPrefDraft.mode === "their" ? "on" : ""} onClick={() => setDmPrefDraft((value) => ({ ...value, mode: "their" }))}><b>{peer.name}의 이야기 속</b><span>상대 캐릭터가 살던 장소에서 만나요.</span></button>
              <button className={dmPrefDraft.mode === "mine" ? "on" : ""} onClick={() => setDmPrefDraft((value) => ({ ...value, mode: "mine" }))}><b>{char.name || "내 캐릭터"}의 이야기 속</b><span>내 캐릭터가 살던 장소에서 만나요.</span></button>
              <button className={dmPrefDraft.mode === "bridge" ? "on" : ""} onClick={() => setDmPrefDraft((value) => ({ ...value, mode: "bridge" }))}><b>둘만의 중립 공간</b><span>서로의 설정을 바꾸지 않고 만나요.</span></button>
            </div>
  ...
  return (
    <>
      ...
      <div className="al-world-options">
        <button onClick={() => chooseDmWorldMode("their")}><b>{pendingDm.peer.name}의 이야기 속</b><span>상대 캐릭터가 살던 장소에서 만나요.</span></button>
        <button onClick={() => chooseDmWorldMode("mine")}><b>{char.name || "내 캐릭터"}의 이야기 속</b><span>내 캐릭터가 살던 장소에서 만나요.</span></button>
        <button onClick={() => chooseDmWorldMode("bridge")}><b>둘만의 중립 공간</b><span>서로의 설정을 바꾸지 않고 만나요.</span></button>
      </div>
    </>
  );
}
...
function ChatKindStep({ finishDmChatKind, pendingDm }) {
  return (
    <>
      ...
      <div className="al-world-options">
        <button onClick={() => finishDmChatKind("npc")}><b>나만 보는 대화</b><span>상대 사용자에게 보이지 않는 개인 대화예요.</span></button>
        <button onClick={() => finishDmChatKind("shared")}><b>상대와 함께 보는 대화</b><span>상대 사용자도 같은 대화를 볼 수 있어요.</span></button>
      </div>
    </>
  );
}
