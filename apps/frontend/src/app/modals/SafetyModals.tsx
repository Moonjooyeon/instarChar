...
function ConsentModal({ ctx }) {
  ...
  return (
    <div className="al-modal-bg al-safety-layer">
      <div className="al-modal al-consent-modal">
        ...
        <button className="al-modal-save al-consent-accept" onClick={acceptTerms}>동의하고 계속</button>
...
function ReportModal({ ctx }) {
  ...
  return (
    <div className="al-modal-bg al-safety-layer" onClick={() => setReportTarget(null)}>
      <div className="al-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <select className="al-report-select" value={reason} onChange={(event) => setReason(event.target.value as ReportReason)}>
          {REASONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
        </select>
        <textarea className="al-report-detail" value={detail} onChange={(event) => setDetail(event.target.value)} maxLength={2000} placeholder="상세 내용을 입력해 주세요. (선택)" />
        <div className="al-modal-actions">
          <button className="al-modal-cancel" onClick={() => setReportTarget(null)}>취소</button>
          <button className="al-modal-danger" onClick={() => submitReport(reason, detail)}>신고 접수</button>
        </div>
      </div>
    </div>
  );
