...
MonoGPT Gemini remains the primary backend-only provider. Cafe24 LLM Router is used only as a backend fallback when its server-only key is configured and MonoGPT is unavailable. The old Vercel serverless `/api/generate` handler has been replaced by FastAPI.
...
- `MONOGPT_GEMINI_API_KEY`
- `MONOGPT_GEMINI_MODEL_FAST`, `MONOGPT_GEMINI_MODEL_GOOD`
- `CAFE24_LLM_API_KEY`, `CAFE24_LLM_MODEL_FAST`, `CAFE24_LLM_MODEL_GOOD`
- `API_DAILY_LIMIT`, `API_PAID_DAILY_LIMIT`, `API_MONTHLY_COST_LIMIT_USD`, `API_ESTIMATED_CALL_COST_USD`
