        ...
        <div className="al-phone">
          ...
          {hasSupabaseConfig && (
            <div className="al-disc-status">
              <span>{sharedLoadState.loading ? "사용자 캐릭터 불러오는 중" : `사용자 캐릭터 ${sharedCharacters.length}개 불러옴`}</span>
          ...
          <div className="al-disc-list">
            {list.length === 0 && <p className="al-disc-none">{sharedFocusId ? "이 공유 링크의 캐릭터를 찾지 못했어." : discoverQuery ? `"${discoverQuery}"에 맞는 새 캐릭터가 없어.` : sharedCharacters.length > 0 ? "팔로우하지 않은 새 캐릭터가 없어." : "아직 공유된 사용자 캐릭터가 없어."}</p>}
            {list.map((c) => {
...
.al-disc-status{ display:flex; align-items:center; justify-content:space-between; gap:10px; margin:0 14px 10px; padding:8px 10px;
