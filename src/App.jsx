...
function App() {
  ...
  async function syncStructuredState(snapshot) {
    ...
    const dmRows = Object.entries(snapshot.dmThreads || {}).map(([threadKey, messages]) => ({
      owner_id: ownerId,
    ...
    if (dmRows.length) {
      jobs.push(supabase.from("alive_dm_threads")
        .upsert(dmRows, { onConflict: "owner_id,thread_key" }));
    }
  ...
  async function loadStructuredStateFallback(baseState, ownerId) {
    ...
    try {
      const [charsResult, personasResult, dmResult] = await Promise.allSettled([
  ...
  function dmKeyFor(peerObj, speakerValue = speakAs) {
    if (!peerObj) return "";
    const base = peerObj.asOwner
      ? `${ownerLabel}|${char.name}`
      : [speakerNameFor(speakerValue), peerObj.name].sort().join("|");
    return `${dmScope}::${base}`;
  }
...
- **반드시 위 "말투 특징"을 그대로 지켜라. 어미·말버릇·문체가 이 캐릭터답게 일관돼야 전한다. 말투가 무너지면 실패다.**
...
${formatRule}${ANTI_REPEAT_RULES}${recentLinesBlock(posts.slice(0, 6).map((p) => p.text))}${correctionBlock()}`;
    ...
    try {
      ...
      // 업로드된 그림 첨부 (일상/셀카/랜덤이면 항상 하나 사용)
...
이건 캐릭터가 아니라 AI 말투다. 캐릭터는 자기 성격대로 무심하게, 퉁명스럽게, 엉뚱하게, 날카롭게 — 뭐든 자기답게 반응한다. 상대를 위로하거나 분석하거나 인생 교훈을 주려 들지 마라. 그냥 지금 이 순간의 대화에 사람처럼 반응하라.`;
  ...
  function relationBaseFor(fromName, toName) {
    ...
    if (isSpecialRelation(hit.label) && !canActivateSpecialRelation(fromName, toName)) return null;
  ...
  async function sendDM() {
    ...
    const senderDesc = senderIsOwner
      ...
      : activePersona
        ...
        : `"${meName}"은(는) 다음 캐릭터다 — ${char.persona || char.name}.${char.world ? ` 세계관: ${char.world}.` : ""}${char.speech ? ` 말투: ${char.speech}.` : ""}`;
...
- 본문만 출력.${ANTI_REPEAT_RULES}`;
  ...
  function submitUserComment(postId, postAuthorName) {
    ...
    // 답할 캐 정하기:
    //  - 외부 캐 글이면 그 글쓴이가 답
    //  - 내 캐릭터 글이면 글쓴이(내 캐릭터) 본인이 답 (자기 글엔 자기가 주인)
    //    단 내가 '캐릭터로' 댓글 단 거면 자문자답이 되니, 그땐 팔로우 캐가 답
    let responder = null;
    if (postAuthorName) {
      ...
      // 캐릭터로 자기 글에 댓글 → 팔로우 캐가 답
      const responders = myFollowers();
      responder = responders[Math.floor(Math.random() * responders.length)];
    }
  ...
  async function followersReactTo(postId, postText) {
    ...
    for (let i = 0; i < reactors.length; i++) {
      ...
      // 글쓴이(내 캐릭터)가 자기 글 댓글에 답하는 경우 — 자기 글이니 자주 끼어든다(65%)
...
.al-dmbtn{ flex-shrink:0; padding:8px 15px; border-radius:20px; cursor:pointer; font-family:inherit;
  ...
  color:#fff; border:none; transition:.16s; }
