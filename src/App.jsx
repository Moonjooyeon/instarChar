...
function App() {
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      setSaveStatus(error ? `저장 실패: ${error.message}` : "저장됨");
    }, 700);
  ...
  function chooseDmWorldMode(mode) {
    ...
    if (mode === "bridge") {
      setDmWorldPrefs((prev) => ({ ...prev, [key]: { mode, note: "" } }));
  ...
  function finishDmWorldSetup(skipNote = false) {
    ...
    const key = dmKeyFor(pendingDm.peer, pendingDm.speakAs);
    setDmWorldPrefs((prev) => ({ ...prev, [key]: { mode: pendingDm.mode, note: skipNote ? "" : dmWorldDraft.trim() } }));
    enterDm(pendingDm.peer, pendingDm.speakAs);
  }
...
아래 항목을 갖춘 JSON 객체로만 답해. 절대 마크다운 백틱(\`\`\`)을 쓰지 마라.
    ...
    try {
      ...
      setChar((prev) => ({
        ...
        handle: normalizeHandle(obj.handle, obj.name),
...
${formatRule}${ANTI_REPEAT_RULES}${recentLinesBlock(posts.slice(0, 6).map((p) => p.text))}${correctionBlock()}`;
  ...
  function addManualMemory() {
    ...
    updateLorebook((list) => [...list, { id: Date.now(), content, peer }].slice(-60));
    setMemDraftText("");
...
- 지문(괄호 안 행동)은 역극에 쓸 법하면 약간만.${ANTI_REPEAT_RULES}${peerChar ? loreBlockFor(peerChar, meName) : ""}${peerChar ? correctionBlockFor(peerChar) : ""}`;
    ...
    try {
      ...
      setDmThread((d) => [...d, { from: peerName, text: `(…답장이 끊겼어. ${e.message})` }]);
    } finally {
      setDmSending(false);
    }
  }
              ...
              {showMemory && (
                <div className="al-memlist">
                  {(char.lorebook || []).length === 0 ? (
                    ...
                    const shown = allMem.filter((e) => (e.peer || "*") === memFilter).slice(-20).reverse();
                    return (
                      <>
                        {!memFilter ? (
