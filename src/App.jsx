...
function App() {
  ...
  async function loadSharedFollowers(sharedId = activeSharedId) {
    ...
    const { data, error } = await supabase
      .from("alive_character_follows")
      .select("id,follower_name,follower_account_id,follower_character,created_at")
    ...
    const rows = (data || []).map((row) => {
      ...
      return {
        ...
        handle: c.handle || "",
        owner: `@${row.follower_name || "user"}`,
        ownerName: row.follower_name || "user",
  ...
  async function shareCurrentCharacter() {
    ...
    const payload = {
      ...
      character: { ...char, posts: publicPostSnapshot() },
    };
    const { data, error } = await supabase
      .from("alive_shared_characters")
          ...
          <div className="al-profile">
            ...
            <div className="al-profile-info">
              ...
              <div className="al-follow-stats">
                ...
                <button className={`al-fstat ${followPanel === "followers" ? "on" : ""}`} onClick={() => {
                  setFollowPanel((v) => {
                    const next = v === "followers" ? null : "followers";
