...
function App() {
  ...
  async function sendLoginLink() {
    ...
    setAuthMessage(error ? error.message : "메일로 로그인 링크를 보냈어. 같은 브라우저에서 열면 이어서 시작돼.");
  }
      ...
      {hasSupabaseConfig && !authLoading && !session && (
        <div className="al-phone">
          <div className="al-auth">
            <span className="al-spark">✶</span>
            <h1>ALIVE 로그인</h1>
            <p>캐릭터, 로어북, DM을 계정에 저장하려면 이메일로 시작해.</p>
            <input className="al-auth-input" type="email" value={authEmail}
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && !e.nativeEvent.isComposing) sendLoginLink(); }} />
            <button className="al-auth-btn" disabled={!authEmail.trim() || authLoading} onClick={sendLoginLink}>
              로그인 링크 받기
            </button>
...
.al-auth-input{ width:100%; background:#1a1a20; border:1px solid var(--line); border-radius:12px;
  padding:13px 14px; color:var(--ink); font-family:inherit; font-size:14px; }
