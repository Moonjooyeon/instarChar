...
function App() {
  ...
  }, [accounts, activeId, char, gallery, posts, personas, dmThreads, dmWorldPrefs, ownerPersona, following, affinity, discoverQuery, profileName, onboardingOpen, step, stateReady, session?.user?.id]); // eslint-disable-line
  ...
  }, [accounts, activeId, char, gallery, posts, personas, dmThreads, dmWorldPrefs, ownerPersona, following, affinity, discoverQuery, profileName, onboardingOpen, stateReady]); // eslint-disable-line
...
{"aff_a_to_b": 0, "aff_b_to_a": 0, "mem_a": ["기억할 내용"], "mem_b": []}
기억할 게 없으면 빈 배열.`;
...
- 지문(괄호 안 행동)은 역극에 쓸 법하면 약간만.${ANTI_REPEAT_RULES}${peerChar ? loreBlockFor(peerChar, meName) : ""}${peerChar ? correctionBlockFor(peerChar) : ""}`;
    ...
    try {
      ...
      if (dmRequestSeqRef.current !== requestId || dmKeyRef.current !== requestKey) return;
      const message = e.name === "AbortError" ? "응답 시간이 너무 길어서 중단됐어. 다시 보내줘." : e.message;
      setDmThread((d) => [...d, { from: peerName, text: `(…답장이 끊겼어. ${message})` }]);
    } finally {
...
${relationHint ? `${listener.name}와의 관계: ${relationHint}` : `${listener.name}와 특별한 관계 없음.`}${worldBridgeBlock(speaker, listener, worldPref)}
                          ...
                          <>
                            ...
                            {shown.map((e) => (
                              <div className="al-mem-edit" key={e.id}>
                                <div className="al-mem-meta">
                                  <button className={e.pinned ? "on" : ""} onClick={() => updateMemory(e.id, { pinned: !e.pinned })}>고정</button>
                                  <span>{e.source === "manual" ? "수동" : "자동"}</span>
                                  <select value={e.importance || 2} onChange={(ev) => updateMemory(e.id, { importance: Number(ev.target.value) })}>
                                ...
                                <button onClick={() => deleteMemory(e.id)}>삭제</button>
                              </div>
                            ))}
                            <div className="al-mem-add">
      ...
      {canUseApp && step === "dm" && peer && (() => {
        ...
        <div className="al-phone">
          <div className="al-dmhead">
            ...
            <div className="al-dmhead-info">
              <span className="al-dmhead-name">{peer.asOwner ? `${peerName} (내 캐릭터)` : peerName}</span>
      ...
      {canUseApp && followPanel && !publicProfile && (
        <div className="al-modal-bg" onClick={() => setFollowPanel(null)}>
          <div className="al-follow-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {(() => {
              const list = followPanel === "following" ? following : myFollowers();
              return list.length === 0 ? (
                <p className="al-follow-empty">{followPanel === "following" ? "아직 팔로우한 캐릭터가 없어." : "아직 맞팔한 캐릭터가 없어."}</p>
              ) : (
...
/* profile DM button */
.al-profile-top{ display:grid; grid-template-columns:minmax(0, 1fr) minmax(156px, 1.24fr); align-items:start; gap:10px; min-width:0; }
.al-dmbtn{ flex:0 1 auto; min-width:0; min-height:36px; padding:8px clamp(7px, 2vw, 15px); border-radius:20px; cursor:pointer; font-family:inherit;
  font-size:clamp(11px, 3.1vw, 13px); font-weight:700; background:linear-gradient(135deg,var(--accent),var(--accent2));
  color:#fff; border:none; transition:.16s; white-space:normal; overflow-wrap:anywhere; line-height:1.2; }
.al-dmbtn:hover{ filter:brightness(1.1); }
.al-feed-actions{ display:flex; justify-content:flex-end; gap:clamp(4px, 1.6vw, 7px); min-width:0; max-width:100%; }
...
@media (max-width:430px){
  ...
  .al-profile-top{ grid-template-columns:minmax(0, .8fr) minmax(142px, 1.2fr); gap:8px; }
  .al-profile-info h2{ font-size:19px; overflow-wrap:anywhere; }
  .al-handle{ display:block; overflow-wrap:anywhere; font-size:12.5px; }
  .al-dmbtn{ min-height:34px; }
  .al-banner{ height:86px; }
  .al-avatar{ width:66px; height:66px; margin-top:-33px; margin-left:12px; }
...
@media (max-width:320px){
  .al-profile-top{ grid-template-columns:minmax(0, .68fr) minmax(132px, 1.22fr); gap:6px; }
  .al-dmbtn{ padding-left:6px; padding-right:6px; font-size:10.5px; }
}
