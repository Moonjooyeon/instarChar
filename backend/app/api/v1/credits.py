...
from app.core.credit_products import CREDIT_PRODUCTS, FIRST_PURCHASE_BONUS_PERCENT, CreditProduct, credit_product_skus, google_play_iap_purchase_available, google_play_product_ids, toss_iap_purchase_available
...
async def get_credit_catalog(user: User = Depends(get_current_user), settings: Settings = Depends(get_settings)) -> CreditCatalogResponse:
    ...
    return CreditCatalogResponse(credit_policy_version=CREDIT_POLICY_VERSION, energy_policy_version=ENERGY_POLICY_VERSION, offers=_offers(settings, toss_available, google_play_available), flows=_flows())
...
def _offers(settings: Settings, purchase_available: bool = False, google_play_purchase_available: bool = False) -> list[CreditOfferResponse]:
    ...
    return [_offer(product, skus[product.offer_id] if integrated else "", purchase_available, google_play_ids[product.offer_id] if google_play_integrated else "", google_play_purchase_available) for product in CREDIT_PRODUCTS]


def _offer(product: CreditProduct, sku: str, enabled: bool, google_play_product_id: str, google_play_enabled: bool) -> CreditOfferResponse:
    total = product.total_credits
    return CreditOfferResponse(id=product.offer_id, sku=sku, google_play_product_id=google_play_product_id, price_krw=product.price_krw, base_credits=product.base_credits, product_bonus_credits=product.product_bonus_credits, first_purchase_bonus_percent=FIRST_PURCHASE_BONUS_PERCENT, total_credits=total, first_purchase_total_credits=total + product.first_purchase_bonus_credits, label=product.label, payment_available=enabled and bool(sku), google_play_payment_available=google_play_enabled and bool(google_play_product_id))
