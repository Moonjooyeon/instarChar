...
class FeedGenerationService:
    ...
    async def generate(self, user: User, source_account_id: str, payload: FeedPostGenerateRequest, is_auto: bool = False) -> GenerateApiResult:
        character = await self.posts.owned_character(user.id, source_account_id)
        request = self._request(character.name, character.character, character.posts, payload.mood)
        result = await self.ai.generate(request, user.id)
        if result.status_code != 200:
            if is_auto:
                await self._record_auto_failure(user, source_account_id, result)
            return result
        ...
        state = await self.posts.append_generated_post(user.id, source_account_id, post, is_auto=is_auto)
        return GenerateApiResult(200, {"post": post, "state": state.model_dump(mode="json")})

    async def _record_auto_failure(self, user: User, source_account_id: str, result: GenerateApiResult) -> None:
        code = str(result.body.get("error") or "GENERATION_FAILED")
        message = str(result.body.get("message") or code)
        retry_at = self._limit_retry_at(code)
        await self.posts.record_auto_failure(user.id, source_account_id, f"{code}: {message}", retry_at)

    def _limit_retry_at(self, code: str) -> datetime:
