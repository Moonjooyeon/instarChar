...
이 가이드는 Google Play 소비성 일회성 크레딧 상품의 구매·취소·환불 상태를 ALIVE 백엔드에 전달하는 Real-time developer notifications(RTDN) 설정과 운영 절차를 설명한다. Google Play Console에서 환불한 주문의 크레딧을 자동 회수하려면 Google Play, Cloud Pub/Sub, ALIVE 백엔드 세 경계를 모두 연결해야 한다.
...
- [ ] Play Console에서 전액 환불한 뒤 `voided` 이벤트가 기록된다.
...
1. Play Console 또는 Google Play Voided Purchases API에서 환불 완료 상태를 확인한다.
