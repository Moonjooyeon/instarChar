...
> **Historical policy notice — 2026-08-11**: 이 보고서의 다섯 단계 DM 가격과 이미지 이해 flow는 현재 제품 정책이 아니다. 현재 DM 정책은 기본 1C·기억 반영 2C·중요한 답장 5C이며, DM 사진 첨부와 이미지 이해 flow는 제거됐다. 최신 범위는 [DM 피드백 단순화 계획](../../../plans/product/app-flow/plan_dm-feedback-simplification_2026-08-11.md)을 따른다.

- 작성일: 2026-08-09
- 범위: 현재 워크트리의 크레딧/에너지/AI 호출 구현, 상품 초안, 결제 전 보안 경계
- 결론 성격: 출시 전 의사결정용 리뷰. 실제 사용자 토큰 분포가 아직 기록되지 않으므로 손익 확정본이 아니다.

> **가격 결정 업데이트 — 2026-08-10**: 이 보고서의 8~18C 제안은 보수적 상한 시나리오다. 제품 결정은 초기 베타 대화 가격 `1C / 2C / 2C / 5C / 7C`이며, 결제를 비활성화한 채 shadow billing p95·p99로 재검토한다.

> **콘솔 판매가 업데이트 — 2026-08-11**: Apps in Toss 실제 판매가는 `4,950 / 9,900 / 29,700 / 49,500 / 99,000원`으로 등록됐다. 아래의 `5,000 / 10,000 / 30,000 / 50,000 / 100,000원` 기반 계산은 역사적 검토값이며 실제 출시 승인에는 1% 낮은 판매가로 다시 계산한 수익성 표를 사용한다.
...
- 제품 결정에 따라 초기 베타는 기본 1C, 문맥 2C, 긴 기억·관계형 2C, Pro 5C, Pro 서사형 7C를 적용한다.
- 기본·문맥·긴 기억 Flash는 token/thinking 상한을 서버 정책으로 제한한다.
- 피드 3C, 이미지 이해 5C, 캐릭터 상호작용 5C는 일반 사용 기준으로 여유가 있으나 p95 실측 후 확정한다.
- Pro DM `8~10C`와 Pro 서사형 `16~18C`는 현재 가격이 아니라 shadow billing에서 비교할 보수적 대안이다.
...
가장 먼저 해야 할 일은 **서버 authority와 실제 token cost를 잠그는 것**이다. 초기 베타 가격은 `1C / 2C / 2C / 5C / 7C`로 운영하되, 결제 활성화 전 shadow billing 데이터로 유지·인상 여부를 결정한다.
