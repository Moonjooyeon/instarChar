---
title: ALIVE 보고서 인덱스
author: black (black@ashwoodfriends.com)
created: 2026-08-06
updated: 2026-08-06
version: 1.0.0
status: approved
---

# 보고서

보고서는 조사·감사·리뷰·운영 결과를 남긴다. 구현 전 문서는 `plans/` 또는 `specs/`, 기기·브라우저 결과는 `qa/`에 둔다.

## 앱인토스

- [앱인토스 보고서 모음](apps-in-toss/)

## 프론트엔드 구조·마이그레이션

- [모노레포 Phase 0 기준선](report_monorepo-frontend-phase0-baseline_2026-07-03.md)
- [모노레포 Phase 1–5 리뷰](report_monorepo-frontend-phase1-review_2026-07-03.md)
- [모노레포 Phase 6 AI generation](report_monorepo-frontend-phase6-ai-generation_2026-07-03.md)
- [모노레포 Phase 6 domain hooks](report_monorepo-frontend-phase6-domain-hooks_2026-07-03.md)
- [모노레포 Phase 6 navigation/persistence](report_monorepo-frontend-phase6-navigation-persistence_2026-07-03.md)
- [모노레포 Phase 7 test harness](report_monorepo-frontend-phase7-test-harness_2026-07-10.md)
- [모노레포 Phase 8 TypeScript](report_monorepo-frontend-phase8-domain-typescript_2026-07-10.md)
- [API client boundary](report_monorepo-frontend-api-client-boundary_2026-07-10.md)
- [Controller style/types](report_monorepo-frontend-controller-style-types_2026-07-10.md)
- [Large screen split](report_monorepo-frontend-large-screen-split_2026-07-10.md)

## 테마·UI

- `report_theme-tailwind-*.md` — 테마·Tailwind 전환 Phase 보고서 모음
- [테마 정적 완료 감사](report_theme-tailwind-static-completion-audit_2026-08-05.md)
- [상단 아이콘 토글](report_theme-top-icon-toggle_2026-08-05.md)

세부 Phase 보고서는 파일명에 `report_theme-tailwind-phase` prefix를 사용한다. 새 보고서는 이 인덱스의 적절한 그룹에 추가한다.

## 백엔드·운영·출시

- [크레딧 가격 및 AI 사용량 정책 분석](report_credit-pricing-analysis_2026-08-06.md)
- [자율 게시글 스케줄러 운영](report_auto-post-scheduler-operations_2026-07-23.md)
- [캐릭터 아이디 검증](report_character-handle-uniqueness-verification_2026-07-30.md)
- [캐릭터 persistence audit](report_character-persistence-audit_2026-08-05.md)
- [다크 모드 단일 운영 전환](report_dark-only-theme-lock_2026-08-05.md)
- [모바일 스토어 readiness](report_mobile-store-readiness_2026-07-24.md)
- [UGC moderation operations](report_ugc-moderation-operations_2026-07-24.md)
