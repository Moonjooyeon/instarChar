...
- [ ] **T8 (P2, human: ~1-2 days / CC: ~2h)** — Gradual TypeScript — domain과 API boundary부터 `.ts/.tsx`로 전환한다.
  ...
  - 진행: TypeScript devDependency와 `npm run typecheck` 추가. `asyncUtils`, `textUtils`, `dmKeyUtils`, `feedUtils`, `affinityUtils`, `relationshipFollowUtils`, `discoverUtils`를 `.ts`로 전환. `aliveCore`를 `.ts`로 전환. `/api/generate` 호출과 응답 mapper를 `src/api/generate.ts` 경계로 분리. `npm run typecheck`, `npm run test:domain`, `npm run test:e2e -- --list`, `npm run build` 성공.
