...
- [ ] **T8 (P2, human: ~1-2 days / CC: ~2h)** — Gradual TypeScript — domain과 API boundary부터 `.ts/.tsx`로 전환한다.
  ...
  - 진행: TypeScript devDependency와 `npm run typecheck` 추가. `asyncUtils`, `textUtils`, `dmKeyUtils`, `feedUtils`, `affinityUtils`, `relationshipFollowUtils`, `discoverUtils`를 `.ts`로 전환. `npm run typecheck`, `npm run test:domain`, `npm run test:e2e -- --list`, `npm run build` 성공.
